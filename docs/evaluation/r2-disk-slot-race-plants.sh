...
# newly available slot. Only the five already-dispatched scheduled walks and
...
setsid taskset -c "$disk_test_controller_cpus" env \
  ...
  r2_monotonic_now_ns() {
    if [[ ${FUNCNAME[1]:-} == wait_for_launch_slot && $injected -eq 0 ]]; then
      injected=1
      r2_publish_decimal_control_marker "$stop" "$((origin + 200000000))"
      : >"$release"
  ...
  r2_read_allowed_cpu_list() {
    if [[ ${FUNCNAME[1]:-} == pool_worker_identity_stable &&
      ...
      ${launch_kind:-} == scheduled && $ordinal -eq 5 ]]; then
      : >"$selection"
      return 2
    fi
  ...
  r2_record_checkout_mib() {
    ...
    if [[ $kind == scheduled && $ordinal -lt 5 ]]; then
...
[[ $slot_stop_status -eq 0 && ! -s $temporary/slot-stop.stdout &&
  ...
  $slot_stop_scheduled -eq 5 &&
  $(tail -n 1 "$slot_stop_samples") == $'5\t1250000000\t250000000\t17\tterminal' ]] ||
...
# Begin a drain with live work, fill the five-walk gate, and advance only gate
...
# session first and no sixth walk or ledger row may be published.
