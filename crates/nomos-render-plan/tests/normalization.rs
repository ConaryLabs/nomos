...
fn the_schema_field_is_ignored() {
    ...
    let rust = br#"{"deterministic":true,"schema":"nomos.rendering_plan@1"}"#;
...
#[test]
fn numbers_compare_by_value_and_not_by_spelling() {
    assert!(normalized_differences(br#"{"h":4.50}"#, br#"{"h":4.5}"#).is_empty());
    assert_eq!(
        normalized_differences(br#"{"h":4.5}"#, br#"{"h":4.6}"#).len(),
        1
    );
}
