...
use estate_core::{Diagnostic, RepairClass, SourcePath, SourceSpan};
...
fn a_diagnostic_carries_code_message_span_and_repairs() {
    ...
    assert_eq!(
        ...
        "EK0104: `north gate` is not an entity id (fixtures/gaol.estate:7:3)"
    );
}
...
fn absolute_paths_cannot_enter_a_diagnostic() {
    ...
    assert!(SourcePath::new("fixtures/gaol.estate").is_ok());
}

#[test]
fn an_inverted_span_is_refused() {
    let path = SourcePath::new("fixtures/gaol.estate").unwrap();
...
fn diagnostics_render_as_canonical_json() {
    let diagnostic = Diagnostic::new(codes::CANONICAL_NOT_CANONICAL, "not canonical")
        .with_span(SourceSpan::new(SourcePath::new("a.estate").unwrap(), 1, 2, 3, 4).unwrap())
        .with_repair(RepairClass::EmitCanonicalBytes);
    let bytes = diagnostic.to_canonical().to_canonical_bytes();
    assert_eq!(
        String::from_utf8(bytes).unwrap(),
        r#"{"code":"EK0303","message":"not canonical","repairs":["emit_canonical_bytes"],"span":{"byte_end":2,"byte_start":1,"column":4,"line":3,"path":"a.estate"}}"#
    );
}
