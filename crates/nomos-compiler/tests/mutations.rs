...
use nomos_compiler::{compile_source, diagnostics, link_source};
use nomos_core::{Diagnostic, SchemaId, SourcePath, SourceSpan};
use nomos_schema::{SourceDocument, Spanned};

const SOURCE: &str = include_str!("../../../fixtures/gaol.nomos");
const PATH: &str = "fixtures/mutated-gaol.nomos";
...
fn source_schema_is_mandatory_exactly_once() {
    reject(
        &SOURCE.replacen("schema nomos.source@1\n", "", 1),
        diagnostics::SOURCE_SCHEMA_REQUIRED.as_str(),
    );
    reject(
        &SOURCE.replacen(
    ...
    reject(
        &SOURCE.replacen("nomos.source@1", "nomos.source@2", 1),
...
fn linker_rechecks_schema_for_callers_that_supply_an_ast_directly() {
    ...
    let document = SourceDocument::new(
        Spanned::new(SchemaId::new("nomos.source", 2).unwrap(), span),
        Vec::new(),
        Vec::new(),
        Vec::new(),
        Vec::new(),
...
fn every_compiler_code_is_well_formed_and_globally_unique() {
    ...
    for code in nomos_core::diagnostic::codes::ALL
        .iter()
