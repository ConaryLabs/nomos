...
fn compile() -> nomos_schema::WorldIr {
...
fn world_ir_bytes_are_canonical_and_repeatable() {
    let ir = compile();
    assert_eq!(ir.schema(), &nomos_schema::construction_world_ir_schema());
    assert_eq!(
...
fn entity<'a>(ir: &'a nomos_schema::WorldIr, id: &str) -> &'a nomos_schema::IrEntity {
    ir.entities()
        .iter()
