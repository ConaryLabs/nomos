...
use nomos_core::ident::{Ident, SEPARATORS};
...
fn schema_versions_start_at_one() {
    assert_eq!(
        SchemaId::new("nomos.source", 0)
            .unwrap_err()
            .code()
            .as_str(),
        "EK0105"
    );
    let schema = SchemaId::new("nomos.source", 2).unwrap();
    assert_eq!(schema.to_string(), "nomos.source@2");
    assert_eq!(
        schema.to_canonical().to_canonical_bytes(),
        br#"{"name":"nomos.source","version":2}"#.to_vec()
    );
}
...
fn schema_ids_parse_their_wire_spelling() {
    assert_eq!(
        SchemaId::parse("nomos.source@1").unwrap(),
        SchemaId::new("nomos.source", 1).unwrap()
    );
    for illegal in ["nomos.source", "nomos.source@", "nomos.source@x"] {
