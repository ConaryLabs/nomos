...
//! ```
//!
//! # Why this lives in `nomos-core`
//!
//! Section 10 gives `nomos-core` "canonical bytes and hashing" and
//! `nomos-projection` "versioned simulation/navigation/persistence/diagnostic
...
//! `nomos-projection` is. `world-ir.json` and `schemas.json` are
//! `nomos-schema` artifacts, and `nomos-schema` may depend only on
//! `nomos-core`; putting the writer in `nomos-projection` would either strand
//! the schema crate or add an edge section 10 forbids. `nomos-core` is the one
...
//! belongs to `nomos-projection`, and it stays there.
//!
//! # What is enforced
//!
...
pub fn manifest_schema() -> SchemaId {
    SchemaId::new("nomos.package.manifest", 1).expect("the manifest schema id is a valid literal")
}
