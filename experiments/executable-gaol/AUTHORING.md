...
- `area.json`: stable area identity, primary exit gate, pursuit light, player
  and gaoler cell anchors, and one `visual/cyan_crescent` presentation anchor.
...
Bounds are `x=0..8`, `y=0..5`, `z=0`. The primary gate must be reachable from
the player anchor, water should create a legible route choice, and the brazier
must light a useful landmark. Change composition and traversal, not the bounded
look. Do not edit `render-core.mjs` for an area.
